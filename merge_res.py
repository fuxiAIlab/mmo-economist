...
merge_res = {
    cfg: {adj: {metric: [] for metric in metrics} for adj in tot_adjs} for cfg in cfgs
}
    ...
    for adj in adjs:
        for seed in seeds:
            for test_seed in test_seeds:
                ...
                res = np.load(
                    os.path.join(
                        baseline, cfg, adj, f"dense_logs_{seed}{test_seed}.npy"
                    ),
                    allow_pickle=True,
                ).item()
                # if len(res[0]['Upgrade'])>2000:continue
                reset_idx = np.where(np.array(res[0]["period_step"]) < 1)[0][
                    1:
                ].tolist() + [len(res[0]["equality"])]
                ...
                for i in range(len(res[0]["Upgrade"])):
                    ...
                    if i == reset_idx[grad_cnt]:
                        mean_graduate.append(
                            np.mean(
                                [
                                    int(each_upgrade[k] > grad_req[grad_cnt])
                        ...
                        grad_cnt += 1
                upgrade = [
                    len(res[0]["Upgrade"][i]) for i in range(len(res[0]["Upgrade"]))
                ]
                tot_upgrade = np.cumsum(upgrade)
                charge = [
                    len(res[0]["Recharge"][i]) for i in range(len(res[0]["Recharge"]))
                ]
                ...
                merge_res[cfg][adj]["last_equality"].append(
                    np.mean(
                        tot_upgrade[reset_idx[-1]] * np.array(equality)[reset_idx[-1]]
                    )
                )
...
for cfg in cfgs:
    for adj in adjs:
        for metric in metrics:
            mean, std = (
                np.mean(merge_res[cfg][adj][metric]),
                np.std(merge_res[cfg][adj][metric]),
            )
    ...
    for adj in ["planner", "ablation_equ", "ablation_pro"]:
        for seed in seeds:
            for test_seed in test_seeds:
                ...
                res = np.load(
                    os.path.join(
                        adj, cfg, "planner", f"dense_logs_{seed}{test_seed}.npy"
                    ),
                    allow_pickle=True,
                ).item()
                # if len(res[0]['Upgrade']) > 2000: continue
