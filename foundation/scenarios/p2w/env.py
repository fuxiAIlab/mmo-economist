...
from foundation.base.base_env import (
    BaseEnvironment,
    resource_registry,
    scenario_registry,
)
from foundation.scenarios.p2w import rewards


@scenario_registry.add
class P2WEnvironment(BaseEnvironment):
    name = "scenarios/p2w"
    agent_subclasses = ["BasicPlayer", "BasicPlanner"]
    required_entities = ["EXP", "MAT", "TOK", "CCY", "LAB", "CAP"]
    ...
    def __init__(
        ...
        *base_env_args,
        ...
        initial_p2w_setting={"EXP": 5, "MAT": 5, "TOK": 5},
        graduation_per_period=100,
        max_period=10,
        timesteps_for_force_refresh=1000,
        adjust_type="none",
        ...
        **base_env_kwargs,
        ...
        self._timesteps_for_force_refresh = timesteps_for_force_refresh
    ...
    def init_map_layout(self):
        self._p2w_setting = {
            k: (
                int(v * self.world_size[0] * self.world_size[1])
                if isinstance(v, float) and (v >= 0 and v <= 1)
                else v
            )
            for k, v in self._curr_p2w_setting.items()
        }

        layouts = {k: [] for k in self._p2w_setting.keys()}
        used_loc = []
        for landmark, n in self._p2w_setting.items():
            while n > 0:
    ...
    def init_agent_personality(self):
        ...
        if self._player_monetary_cost_dist == "pareto":
            ...
            self._player_monetary_cost_sensitivities = np.clip(normal_samples, 0, 1)
        else:
        ...
        if self._player_nonmonetary_cost_dist == "pareto":
            ...
            normal_samples = np.random.normal((1 + 0) / 2, (1 - 0) / 3, self.n_agents)
            self._player_nonmonetary_cost_sensitivities = np.clip(normal_samples, 0, 1)
        else:
            raise NotImplementedError

        for agent in self.world.agents:
            agent.set_cost_sensitivity(
                monetary_cost_sensitivity=self._player_monetary_cost_sensitivities[
                    agent.idx
                ],
                nonmonetary_cost_sensitivity=self._player_nonmonetary_cost_sensitivities[
                    agent.idx
                ],
            )
    ...
    def init_p2w_setting(self):
        ...
        self._metrics_for_periods = {
            "profitability": [0.0],
            "equality": [0.0],
            "final_graduation": [0.0],
            "period_graduation": [0.0],
            "capability_avg": [0.0],
        }
    ...
    def energy_weight(self):
        ...
        if self.energy_warmup_method == "decay":
            return float(-np.exp(-self._completions / self.energy_warmup_constant))
    ...
    def get_current_optimization_metrics(self):
        ...
        for agent in self.world.agents:
            curr_optimization_metric[agent.idx] = rewards.isoelastic_utility_for_player(
                income=agent.state["endogenous"]["CAP"],
                monetary_cost=self.starting_player_currency
                - agent.total_endowment("CCY"),
                nonmonetary_cost=agent.state["endogenous"]["LAB"],
        ...
        if self.planner_reward_type == "utility1":
            ...
            ] = rewards.utility_for_planner(
                monetary_incomes=np.array(
                    [
                        self.starting_player_currency - agent.total_endowment("CCY")
                        for agent in self.world.agents
                    ]
                ),
            ...
            curr_optimization_metric[
                self.world.planner.idx
            ] = rewards.utility_normalized_for_planner(
                monetary_incomes=np.array(
                    [
                        self.starting_player_currency - agent.total_endowment("CCY")
                        for agent in self.world.agents
            ...
            curr_optimization_metric[
                self.world.planner.idx
            ] = rewards.utility2_normalized_for_planner(
                monetary_incomes=np.array(
                    [
                        self.starting_player_currency - agent.total_endowment("CCY")
                        for agent in self.world.agents
    ...
    def reset_agent_states(self):
        ...
        for agent in self.world.agents:
            ...
            # Add starting items
            agent.state["inventory"]["TOK"] = float(self.starting_player_token)
            agent.state["inventory"]["CCY"] = float(self.starting_player_currency)
    ...
    def scenario_step(self):
        ...
        for resource in resources:
            ...
            all_resource_map += np.minimum(resource_map, resource_source_blocks)

        self._steps_in_period += 1
        total_launch = sum([v for _, v in self._p2w_setting.items()])

        metrics = self.scenario_metrics()
        self._metrics_for_timesteps["profitability"].append(
            metrics["social/profitability"]
        )
        self._metrics_for_timesteps["equality"].append(metrics["social/equality"])
        ...
        if (
            np.sum(all_resource_map)
            <= max(1, self._normal_wear_and_tear_rate * total_launch)
            or self._steps_in_period >= self._timesteps_for_force_refresh
        ):
            self._periods += 1
            self._steps_in_period = 0
            self._max_capability_in_last_period = {
                str(agent.idx): agent.endogenous["CAP"] for agent in self.world.agents
            }
            ...
            self._metrics_for_timesteps = {
                k: [] for k, _ in self._metrics_for_timesteps.items()
            }

            if self._adjust_type == "fixed":
                # 固定投放，不调整
                pass
            elif self._adjust_type == "random-asy":
                # 随机投放（异步，即每个物品都随机）
                ...
                if (
                    ...
                    self._curr_p2w_adjust = {
                        ...
                        + adjust_rates[
                            min(
                                int(
                                    adjust_rates.index(round(v - base_adjust[k], 2)) + 1
                                ),
                                len(adjust_rates) - 1,
                            )
                        ]
                    ...
                    if (
                        ...
                        self._curr_p2w_adjust = {
                            ...
                            + adjust_rates[
                                max(
                                    int(
                                        adjust_rates.index(round(v - base_adjust[k], 2))
                                        - 1
                                    ),
                                    0,
                                )
    ...
    def generate_observations(self):
        ...
        obs[self.world.planner.idx].update({"period": self._periods / self._max_period})
    ...
    def check_if_done(self):
        ...
        if (
            self.world.timestep >= self._episode_length
            or self._periods >= self._max_period
        ):
            return True
        else:
            return False
    ...
    def scenario_metrics(self):
        ...
        capability_endowments = np.array(
            [agent.endogenous["CAP"] for agent in self.world.agents]
        )
        ...
        metrics["social/profitability"] = (
            rewards.get_profitability(currency_endowments) / self.world.n_agents
        )
        metrics["social/equality"] = rewards.get_equality(capability_endowments)
        metrics["social/capability_avg"] = np.mean(capability_endowments)
        metrics["social/final_graduation"] = (
            np.sum(
                [
                    1
                    if agent.endogenous["CAP"]
                    >= (self._periods + 1) * self._graduation_per_period
                    else 0
                    for agent in self.world.agents
                ]
            )
