...
def plot_map(maps, locs, ax=None, cmap_order=None):
    world_size = np.array(maps.get(list(maps.keys())[0])).shape
    n_agents = len(locs)
    ...
    scenario_entities = [x for x in maps.keys()]
    for entity in scenario_entities:
        if landmarks.has(entity):
            map_ = (
...
def breakdown(log, remap_key=None):
    ...
    for r, ax in zip(rs, axes):
        for i in range(n):
            ax.plot(
                [x[str(aidx[i])]["endogenous"][r] for x in log["states"]],
                label=i,
                color=cmap(i),
            )
        ax.set_title(r)
