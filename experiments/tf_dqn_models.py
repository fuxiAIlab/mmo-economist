...
class KerasDQNConv(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        for tag in ["_pol", "_val"]:
            ...
            lstm_out = tf.keras.layers.LayerNormalization(name="layer_norm" + tag)(dense)
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # seq_lens=tf.ones(tf.shape(input_dict["obs"]['flat'])[0])
        # padded_batch_size = tf.shape(input_dict["obs"]['flat'])[0]
    ...
    def forward_rnn(self, inputs, state, seq_lens):
        model_out, self._value_out= self.rnn_model(
        # model_out, self._value_out, h_p, c_p, h_v, c_v = self.rnn_model(
            inputs #+ [seq_lens] + state
        )
        return model_out, state,#[h_p, c_p, h_v, c_v]
...
ModelCatalog.register_custom_model(KerasDQNConv.custom_name, KerasDQNConv)
